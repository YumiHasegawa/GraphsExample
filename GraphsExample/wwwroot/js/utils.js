    ...
    return {
        ...
        init: function (dateFormat, isMobileOrTablet, decimalSep) {
            ...
            awe.bfr = function (opt) {
                //[TakeProfit Technology] REMIND THIS CODE ON OMU UPDATES
                if (opt.type !== 'post') {
                    opt.type = 'post'; 
                    //return;
                }
...
//                    pcon = o.i + ' ' + msg; // [TakeProfit Technology] REMIND THIS CODE ON OMU UPDATES: commented out
                }                             // because of displaying notifications with a prefix as id of popup


                //[TakeProfit Technology] REMIND THIS CODE ON OMU UPDATES
                var x = document.getElementsByClassName("o-err");
                for (var i = 0; i < x.length; i++) {
                    if (x[i].innerText == null) {
                        continue;
                    }
                    if (x[i].innerText.includes(msg)) {
                        $(x[i]).remove();
                    }
                }
